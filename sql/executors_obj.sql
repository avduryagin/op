WITH contractdata
     ...
     AS (SELECT c.*
      ...
      INNER JOIN sf_class c
           ON c.ID = e.EXECUTOR_ID AND c.SF_REF_TYPE IN ('PR', 'BR') ),
 executors as (SELECT meropContract.type_id,
       ...
       inner join (SELECT t.ct AS CONTRACTOR_ID,
                   ...
                   FROM   sf_claslink t,
                          ...
                          merop m
                   WHERE  t.ct = e.id AND
                          t.cf = m.id) AS meropContract
               ON meropContract.contractor_id = executorsbyorg.id),             
res as(               
select executors.*,objects.well_id,objects.well_pad_id, objects.field_id,objects.shop_id,enterprise_id from executors  inner join objects 
on executors.id=objects.executor_id)
select * from res 
where (res.type_id in {types}) and ((res.well_id is null or (res.well_id is not null and res.well_id in {wells})) and (res.well_pad_id is null or (res.well_pad_id is not null and res.well_pad_id in {well_pads})) and 
((res.field_id is null) or (res.field_id is not null and res.field_id in {fields})) and ((res.shop_id isnull) or (res.shop_id is not null and res.shop_id in {shops})) and ((res.enterprise_id is null) or (res.enterprise_id is not null and res.enterprise_id  in {enterprises})))
