...
select executors.id as execId,executors.name,merops.id as meropId, merops.type_id, merops.obj_type_code, merops.obj_id, wells.WELL_ID, wells.WELL_PAD_ID, wells.FIELD_ID, wells.SHOP_ID, wells.WELL_PAD_NAME from (SELECT meropContract.merop_id,
       ...
       inner join (SELECT t.ct AS CONTRACTOR_ID,
               ...
               ON meropContract.contractor_id = executorsbyorg.id) executors right join (select * from VSF_ACTIVITY_OPT_PLAN v
